...
- **通用 LLM Provider 架构（v1.3.6+）**：支持 OpenAI 标准 API 格式的 LLM 服务
  ...
  - HTTP 连接池优化，复用连接提升性能
  - 流式响应支持，降低首字延迟
- **历史对话上下文（v1.3.5+）**：意图识别和结果分析阶段支持历史对话
  - intent_history_count：意图识别使用 5 轮历史
  - analysis_history_count：结果分析使用 2 轮历史
...
### 🔌 Bukkit API 动态调用（v1.3.3+）
- **数据驱动的原版 API 调用**：从 `apis.yml` 配置文件加载 API 定义，无需硬编码代码
- **反射执行引擎**：动态调用 Player/World/Server 的各种方法
- **双模式支持**：链式调用（method_chain）和并行调用（additional_methods）
- **权限控制**：每个 API 都有独立的权限节点
- **热重载支持**：修改配置立即生效
- **44+ 内置 API**：覆盖玩家状态、世界信息、服务器信息查询

---

### 🧠 多步骤任务执行器（v1.3.2+）
- **复杂任务自动分解**：基于拓扑排序的依赖关系管理
- **数据自动流转**：前置步骤结果自动传递给后续步骤
- **智能结果分析**：LLM 综合分析所有步骤结果并生成友好回复
- **失败回退机制**：意图识别或技能执行失败时自动转为普通 AI 对话
...
完整示例和详细说明请参考 [Kilacraft-AI-Skill-SPI-接入文档.md](doc/Kilacraft-AI-Skill-SPI-%E6%8E%A5%E5%85%A5%E6%96%87%E6%A1%A3.md)
...
详细的版本更新记录请查看 [Kilacraft-AI- 更新日志.md](doc/Kilacraft-AI-%20%E6%9B%B4%E6%96%B0%E6%97%A5%E5%BF%97.md)
