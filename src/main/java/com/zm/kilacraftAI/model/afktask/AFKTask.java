...
public abstract class AFKTask {
    ...
    private final AFKTaskTypeEnum taskType;
    private final String description;
    private final Map<String, String> params;
    private volatile AFKTaskStatusEnum status;
     ...
     * @param params      任务参数
     */
    protected AFKTask(String taskId, UUID playerUUID, String playerName, AFKTaskTypeEnum taskType, String description, Map<String, String> params) {
        this.taskId = taskId;
        ...
        this.params = params;
        this.status = AFKTaskStatusEnum.PENDING;
    ...
    protected void markRunning() {
        this.status = AFKTaskStatusEnum.RUNNING;
    }
    ...
    protected void complete(String message) {
        synchronized (terminationLock) {
            if (this.status == AFKTaskStatusEnum.COMPLETED || this.status == AFKTaskStatusEnum.CANCELLED) {
                return;
            }
            this.status = AFKTaskStatusEnum.COMPLETED;
        }
        onStop();    // 释放子类资源（事件监听器、定时任务等）——在锁外执行，避免子类回调死锁
        cleanup();
        PluginLoggerUtil.debug("挂机任务", "任务终止: {} - {}", taskId, message);
    }
    ...
    public final void stop() {
        synchronized (terminationLock) {
            ...
            this.status = AFKTaskStatusEnum.CANCELLED;
        }
        onStop();
        PluginLoggerUtil.debug("挂机任务", "任务已取消: {}", taskId);
    }
    ...
    protected void notifyPlayer(String message) {
        ...
        if (player != null && player.isOnline()) {
            // 使用统一响应管线（挂机任务回调场景）
            plugin.getResponsePipeline().send(player, I18nService.tr(message), OutputScenarioEnum.AFK_CALLBACK);
        }
    }
    ...
    protected void notifyWithLLMAnalysis(String eventDescription) {
        ...
        coordinator.outputAnalysisResult(player, summary, context, new ArrayDeque<>(), OutputScenarioEnum.AFK_CALLBACK, false  // 挂机任务回调不显示占位符
        );
    }
    ...
    public boolean isActive() {
        return status == AFKTaskStatusEnum.PENDING || status == AFKTaskStatusEnum.RUNNING;
    }
