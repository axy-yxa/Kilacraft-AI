...
public class InternalEnumRegistry {
    ...
    public void loadAll() {
        ...
        PluginLogger.info("枚举注册", I18nService.tr("已加载 {} 个音效、{} 个粒子、{} 个统计枚举", soundEntries.size(), particleEntries.size(), statisticEntries.size()));
    }
    ...
    private void loadSection(String resourcePath, String section, Map<String, List<String>> target) {
        try {
            ...
            if (is == null) {
                PluginLogger.warn("枚举注册", I18nService.tr("找不到内置资源: {}", resourcePath));
                return;
            }
            ...
            PluginLogger.warn("枚举注册", I18nService.tr("加载内置枚举失败: {} - {}", resourcePath, e.getMessage()));
        }
    }
