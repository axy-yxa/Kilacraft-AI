...
public class PlayerLevelChangeWatchTask extends AFKTask implements Listener {
    ...
    public void onPlayerLevelChange(PlayerLevelChangeEvent event) {
        ...
        if (hasCallback) {
            ...
            complete(I18nService.tr("目标玩家 {} 等级变化，开始执行回调。", changedPlayerName));
            ...
            notifyWithLLMAnalysis(I18nService.tr("目标玩家 {} 等级变化（{} → {}，{}）", changedPlayerName, oldLevel, newLevel, direction));
            complete(I18nService.tr("目标玩家 {} 等级变化（{} → {}），挂机任务完成。", changedPlayerName, oldLevel, newLevel));
        }
    }
    ...
    public String getTaskDescription() {
        if (callback != null && callback.getCallbackTask() != null && callback.getCallbackTask().getSteps() != null && !callback.getCallbackTask().getSteps().isEmpty()) {
            ...
            return I18nService.tr("监视玩家 {} 等级变化，触发回调任务（{}步）{}", targetPlayerName, callback.getCallbackTask().getSteps().size(), goalDesc);
        }
