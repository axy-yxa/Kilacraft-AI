...
public class PlayerBedLeaveWatchTask extends AFKTask implements Listener {
    ...
    public void onPlayerBedLeave(PlayerBedLeaveEvent event) {
        ...
        if (hasCallback) {
            ...
            complete(I18nService.tr("目标玩家 {} 已离开床，开始执行回调。", leavingPlayerName));
            executeCallback(leavingPlayerName, bedLocation);
        } else {
            // 纯通知模式：通过 LLM 二次分析通知
            notifyWithLLMAnalysis(I18nService.tr("目标玩家 {} 已离开床", leavingPlayerName));
            complete(I18nService.tr("目标玩家 {} 已离开床，挂机任务完成。", leavingPlayerName));
        }
    }
