 ...
 * 玩家死亡挂机任务
 *
 * <p>监听指定玩家死亡事件，当目标玩家死亡时触发多步骤回调任务。</p>
 *
 * <h3>设计原则：</h3>
 * <ul>
 *   <li>与 PlayerOnlineWatchTask 完全对称</li>
 *   <li>本任务只负责监视死亡事件，回调逻辑与上线监视一致</li>
 * </ul>
 *
 * <h3>使用场景：</h3>
 * <ul>
 *   <li>"帮我盯着 Steve，他死了告诉我"</li>
 *   <li>"监视玩家A，他死了之后帮我查询死亡坐标"</li>
 * </ul>
 *
 * <h3>必需参数：</h3>
 * <ul>
 ...
 * @since 2026-04-10
 */
public class PlayerDeathWatchTask extends AFKTask implements Listener {
    ...
    public PlayerDeathWatchTask(String taskId, UUID playerUUID, String playerName, String description, Map<String, String> params) {
        super(taskId, playerUUID, playerName, AFKTaskType.PLAYER_DEATH_WATCH, description, params);
    ...
    public void onPlayerDeath(PlayerDeathEvent event) {
        ...
        if (hasCallback) {
            ...
            complete(I18nService.tr("目标玩家 {} 已死亡，开始执行回调。", deadPlayerName));
            executeCallback(deadPlayerName);
        } else {
            // 纯通知模式：通过 LLM 二次分析通知
            notifyWithLLMAnalysis(I18nService.tr("目标玩家 {} 已死亡", deadPlayerName));
            complete(I18nService.tr("目标玩家 {} 已死亡，挂机任务完成。", deadPlayerName));
        }
    }
    ...
    public String getTaskDescription() {
        if (callback != null && callback.getCallbackTask() != null && callback.getCallbackTask().getSteps() != null && !callback.getCallbackTask().getSteps().isEmpty()) {
            ...
            return I18nService.tr("监视玩家 {} 死亡，触发回调任务（{}步）{}", targetPlayerName, callback.getCallbackTask().getSteps().size(), goalDesc);
        }
