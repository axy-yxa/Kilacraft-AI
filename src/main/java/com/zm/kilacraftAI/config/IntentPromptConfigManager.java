...
public class IntentPromptConfigManager {
    ...
    private static final String DEFAULT_ROLE_DEFINITION = """
            你是一个智能的技能意图识别助手，分析用户的自然语言输入，判断是否需要调用技能来完成任务。
    ...
    private static final String DEFAULT_PHASE1_ROLE_DEFINITION = """
            你是两阶段意图识别系统的第一阶段（粗选阶段）。你的输出将传递给第二阶段进行精确的动作选择和参数提取，因此你的目标是粗选——宁多勿漏。
            ...
            - 对话历史中 AI 回复可能带来源标签（如「AI[守护]: 内容」表示守护系统主动提醒）。如果玩家对此类消息表达不满或安静意图（“闭嘴”“太吵了”等），应纳入守护技能（玩家意图是关闭守护，对应 disable 动作）
    ...
    private void loadPromptSections() {
        // 第一部分:角色定义
        this.roleDefinition = config.getString("role_definition", DEFAULT_ROLE_DEFINITION);
        ...
        this.phase1RoleDefinition = config.getString("phase1.role_definition", DEFAULT_PHASE1_ROLE_DEFINITION);
     ...
     */
    private static final String DEFAULT_PENDING_CLASSIFY_PROMPT = """
            请判断玩家本轮回复针对上述【待处理操作】的意图，只输出一个 JSON：
