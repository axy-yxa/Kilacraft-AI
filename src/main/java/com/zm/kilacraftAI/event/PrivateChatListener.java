...
public class PrivateChatListener implements Listener {
    ...
    public void onPlayerCommand(PlayerCommandPreprocessEvent event) {
        ...
        PluginLogger.debug("私聊监听", "{} → {}, 解析到目标玩家: {}", senderName, targetName, message);
        ...
        FoliaCompat.getIOPool().submit(() -> {
            try (Connection conn = databaseManager.getConnection()) {
                ...
                if (profile == null) {
                    PluginLogger.debug("私聊监听", "目标玩家 {} 不在画像数据库中，跳过社交关系记录", targetName);
                    return;
                }

                UUID targetUuid = profile.getUuid();
                if (!targetUuid.equals(senderUuid)) {
                    PluginLogger.debug("私聊监听", "记录社交关系: {} → {} (private_chat)", senderName, targetName);
                    socialGraph.recordInteraction(senderUuid, targetUuid, "private_chat");
                } else {
                    PluginLogger.debug("私聊监听", "发送者和目标相同，跳过: {}", senderName);
                }
            } catch (Exception e) {
                PluginLogger.debug("私聊监听", "解析目标玩家失败: {}", e.getMessage());
            }
        });
    }
