...
public class SuggestionService {
    ...
    public void generateAsync(Player player, Deque<ConversationManager.Message> history, String playerName, OutputScenarioEnum scenario, ConversationSourceEnum source) {
        ...
        provider.processRequestWithCustomSystemPrompt(prompt.userPrompt(), player, recentHistory, handler, prompt.systemPrompt(), false, true, false, CacheCallTypeEnum.SUGGESTION).orTimeout(config.getTimeoutSeconds(), TimeUnit.SECONDS).thenAccept(rawResponse -> {
