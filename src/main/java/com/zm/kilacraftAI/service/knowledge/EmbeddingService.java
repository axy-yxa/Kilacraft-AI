...
public class EmbeddingService {
    ...
    public EmbeddingService(ConfigManager configManager, Path knowledgeDir) {
        ...
        if (isBlank(apiUrl) || isBlank(apiKey) || isBlank(model)) {
            available = false;
            PluginLoggerUtil.info("知识库", "Embedding 未配置，已降级到 BM25 检索");
        } else {
            available = true;
            PluginLoggerUtil.info("知识库", "Embedding API URL: {}", apiUrl);
            PluginLoggerUtil.info("知识库", "Embedding 模型: {}", model);
        }
    }
    ...
    public void precomputeAllChunks(Map<String, List<String>> allChunkCache) {
        ...
        for (int i = 0; i < allChunks.size(); i += batchSize) {
            ...
            if (vectors == null) {
                PluginLoggerUtil.warn("知识库", "Embedding 预计算失败，后续请求将走 BM25 降级");
                saveCacheIfNeeded();
                return;
            }
            ...
            if (computedCount > 0) {
                PluginLoggerUtil.info("知识库", "Embedding 预计算进度：已计算 {} / {} 个片段", computedCount, totalChunks);
            }
        }
        ...
        PluginLoggerUtil.info("知识库", "Embedding 预计算完成：共 {} 个片段，缓存命中 {}，API 新计算 {}，耗时 {}ms", totalChunks, cachedCount, computedCount, elapsed);
    }
        ...
        try {
            ...
            try (Response response = httpClient.newCall(request).execute()) {
                if (!response.isSuccessful()) {
                    ...
                    PluginLoggerUtil.warn("知识库", "Embedding API 调用失败: HTTP {} - {}", response.code(), errorBody.length() > 200 ? errorBody.substring(0, 200) : errorBody);
                    return null;
                }
            ...
            PluginLoggerUtil.warn("知识库", "Embedding API 调用异常: {}", e.getMessage());
            return null;
        }
    }
    ...
    private List<float[]> parseBatchEmbeddingResponse(String responseBody) {
        ...
        try {
            ...
            if (dataArr.isEmpty()) {
                PluginLoggerUtil.warn("知识库", "Embedding API 返回空 data 数组");
                return results;
            }
            ...
            PluginLoggerUtil.warn("知识库", "解析 Embedding API 响应失败: {}", e.getMessage());
        }
        return results;
    }
    ...
    public void clearCache() {
        ...
        available = false;
        PluginLoggerUtil.info("知识库", "Embedding 缓存已清除，需要重新预计算");
    }
    ...
    public void shutdown() {
        try {
            ...
            PluginLoggerUtil.debug("知识库", "关闭 Embedding HTTP 客户端异常: {}", e.getMessage());
        }
