...
public abstract class AbstractEventWatchTask extends AFKTask implements Listener {
     ...
     */
    protected AbstractEventWatchTask(String taskId, UUID playerUUID, String playerName, AFKTaskTypeEnum taskType, String description, Map<String, String> params) {
    ...
    protected void executeCallback(String eventDescription, Consumer<TaskPlan> placeholderReplacer) {
        try {
            ...
            if (creatorPlayer == null || !creatorPlayer.isOnline()) {
                PluginLoggerUtil.warn("挂机任务", I18nService.tr("任务创建者不在线，无法执行回调: {}", getTaskId()));
            ...
            PluginLoggerUtil.error("挂机任务", I18nService.tr("构建回调任务失败: {}", e.getMessage()), e);
            Player errorPlayer = Bukkit.getPlayer(getPlayerUUID());
            if (errorPlayer != null && errorPlayer.isOnline()) {
    ...
    protected void onStop() {
        if (listenerRegistered) {
            try {
                ...
                PluginLoggerUtil.debug("挂机任务", "已停止: {}", getTaskId());
            } catch (Exception e) {
                PluginLoggerUtil.warn("挂机任务", I18nService.tr("注销事件监听器失败: {}", e.getMessage()), e);
            }
        }
    }
