...
public class WeatherChangeWatchTask extends AbstractEventWatchTask {
    ...
    public WeatherChangeWatchTask(String taskId, UUID playerUUID, String playerName, String description, Map<String, String> params) {
        super(taskId, playerUUID, playerName, AFKTaskTypeEnum.WEATHER_CHANGE_WATCH, description, params);
    ...
    public void start() {
        try {
            ...
            PluginLoggerUtil.debug("挂机任务", "已启动: {}, 目标世界: {}, 模式: {}", getTaskId(), worldDesc, hasCallback() ? I18nService.tr("回调({}步)", getCallback().getCallbackTask().getSteps().size()) : I18nService.tr("纯通知"));
        } catch (Exception e) {
