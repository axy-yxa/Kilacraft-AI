...
public class AIResponsePipeline {
     ...
     */
    public void send(Player player, String rawMessage, OutputScenarioEnum scenario) {
        if (player == null || rawMessage == null) {
            return;
        }

        // 统一打印DEBUG日志（所有场景的最终回复都从这里输出）
        PluginLoggerUtil.debug("AI响应", "{} 收到AI回复 [{}]：{}", player.getName(), scenario.name(), rawMessage);
        ...
        // 路由（选择载体）
        OutputChannelEnum channel = resolveChannel(scenario);
    ...
    public void sendError(Player player, String errorMessage) {
        ...
        // 错误消息使用 ERROR 场景配置
        OutputChannelEnum channel = resolveChannel(OutputScenarioEnum.ERROR);
     ...
     */
    public void sendThinking(Player player, String message, OutputChannelEnum channel) {
     ...
     */
    public void startStream(Player player, OutputChannelEnum channel, boolean silent) {
     ...
     * @param channel        输出载体（由调用方传入场景配置）
     */
    public void updateStream(Player player, String chunk, String currentMessage, OutputChannelEnum channel) {
        if (player == null || !config.isStreamEnabled()) {
            return;
        }
     ...
     * @param scenario     输出场景（用于场景级载体配置）
     */
    public void completeStream(Player player, String finalMessage, OutputScenarioEnum scenario) {
        if (player == null || !config.isStreamEnabled()) {
            return;
        }
        ...
        // 非输出副作用（由管线统一管理）
        PluginLoggerUtil.debug("AI响应", "{} 收到AI回复 [{}]：{}", player.getName(), scenario.name(), finalMessage);
        if (soundEffectManager != null) {
            soundEffectManager.playResponseSound(player);
        }
    }
    ...
    public void broadcast(String rawMessage, Player excludePlayer) {
        ...
        OutputChannelEnum channel = OutputChannelEnum.CHAT;
     ...
     * @return 输出载体
     */
    private OutputChannelEnum resolveChannel(OutputScenarioEnum scenario) {
        return config.getChannelForScenario(scenario);
    }
