 ...
 * @since 2026-03-25
 */
public class AIRequestValidatorUtil {

    private final KilacraftAI plugin;

    private final Map<UUID, Long> cooldowns;

    public AIRequestValidatorUtil(KilacraftAI plugin) {
        this.plugin = plugin;
    ...
    public void saveToHistory(Deque<ConversationManager.Message> history, String userMessage, String aiResponse, UUID playerId, String personality) {
        saveToHistory(history, userMessage, aiResponse, playerId, personality, ConversationSourceEnum.PLUGIN);
    }
     ...
     */
    public void saveToHistory(Deque<ConversationManager.Message> history, String userMessage, String aiResponse, UUID playerId, String personality, ConversationSourceEnum source) {
        ...
        while (history.size() > maxHistory * 2) {
            ...
            PluginLoggerUtil.debug("历史管理", "移除最早的历史记录：{}", removed.getContent().substring(0, Math.min(20, removed.getContent().length())) + "...");
        }
        ...
        if (playerId != null && personality != null) {
            ...
            PluginLoggerUtil.debug("历史管理", "已保存 AI 回复到最新回复缓存：{}_{}", playerId, personality);
        }
        ...
        PluginLoggerUtil.debug("历史管理", "已保存新对话，当前历史记录数量：{}", history.size());
    }
