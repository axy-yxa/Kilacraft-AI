...
public class ConfigResourceUtil {
    ...
    public static void saveDefaultResource(KilacraftAI plugin, String resourcePath) {
        ...
        try {
            ...
            PluginLoggerUtil.info(LOG_MODULE, "已创建默认配置文件: {}", targetFile.getName());
        } catch (Exception e) {
            PluginLoggerUtil.error(LOG_MODULE, I18nService.tr("创建配置文件失败: {}", resourcePath), e);
        }
    }
    ...
    public static void saveDefaultResourceDir(KilacraftAI plugin, String resourceDir, int maxDepth) {
        ...
        try {
            ...
            PluginLoggerUtil.error(LOG_MODULE, I18nService.tr("扫描内置资源目录失败: {}", resourceDir), e);
        }
    }
}
