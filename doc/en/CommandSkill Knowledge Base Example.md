...
| Command source | `commands/commands.md` (Chinese/default), `commands/commands_en.md` (English). The plugin reads the file matching the language set in `config.yml` and auto-releases the default document for the current language on first startup |
...
│   └── commands_en.md       # English command document
...
> English clients read `commands/commands_en.md`; the format is identical, only the field names change to `description:` / `example:` / `permission:` / `keywords:` (see the English template generated by the plugin).
...
- ☐ The file is at `commands/commands.md` (English: `commands/commands_en.md`), UTF-8 encoded?
