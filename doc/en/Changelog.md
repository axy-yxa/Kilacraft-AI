...
- **Greeting messages no longer injected into in-memory conversation history**: login greetings are now written only to the database `source` column for write isolation (not re-read when loading history), and no longer injected into the in-memory conversation history — a greeting is an isolated assistant message (no paired user), and injecting it would break the history's paired structure and scatter pairs during trimming. The accompanying "history source tag" feature (the `AI[Greeting]` label) has also been removed
...
- **Greeting system rebuilt**: login greetings no longer inject raw vanilla statistics (playtime, distances, kill counts, etc.) or last coordinates; instead only highlight events from the last offline period (kills, boss kills, etc.) are kept. The tone shifts from "status report" to "casual recall" — the AI brings up your past exploits naturally, like an old friend. Removing the high-frequency dynamic parameters also stabilizes the greeting-prompt prefix and **significantly boosts cache hit rate on this path**
