...
- **Web Search (highlight of this release)**: The in-game AI can now look up real-time information on the internet — Minecraft version updates, mod recommendations, today's gold price / exchange rates, sports scores, wiki entries — no longer limited by training-data cutoff. **9 search-engine providers** built in, auto-selected by server language:
  - 5 China providers: Zhipu AI, Baidu Qianfan, Volcengine Doubao, Qiniu Baidu, Alibaba Cloud IQS
  - 4 international providers: Tavily, Brave, Exa, You.com
  - Most offer free tiers, so you can start at zero cost; multiple can be configured as backups. Server owners fill in one API Key in `web.yml` to get started; `provider: auto` routes by language, or pin a specific one. Supports time-range filtering (today / past week / past month) and automatic multi-step search (complex questions split into up to 5 sub-searches). Requires `kilacraft.websearch` permission (default: all players, but requires server owner to configure an API Key to function)

- **Web Fetch**: Hand the AI a specific URL and it fetches the page body, reads it, and answers questions about that page — e.g. "what does this tutorial page say" or "what's the crafting recipe on this mod's wiki page." Complementary to Web Search: search gives keywords → page list; fetch gives a URL → reads the body. **Zero-config, no API Key needed**, works out of the box. Enterprise-grade security built in (see "Security Hardening" below). Requires `kilacraft.webfetch` permission (default: all players)
...
- **Player Custom Watches (WatchSkill)**: Set an "auto-watch" with a single natural-language sentence; when the condition is met or the event fires, the AI proactively notifies you. Replaces the removed AFK-task system with stronger, safer capabilities:
  - **Condition watch**: watch a number or state — "watch my iron ingots until 64", "remind me when health drops below 30%", "notify me when balance hits 10000". Supports probing built-in skills' read-only queries (Bukkit stats, CMI, market queries, vanilla API); value types (number/boolean/string) are auto-detected at runtime
  - **Event watch**: watch 11 high-value game events — furnace smelt complete, crop mature, entity killed (boss), nearby entity spawn, player death, teleport, level change, world change, block break, fishing, chat keyword
  - On trigger it **notifies the AI only, does not auto-execute actions** (safer than the old AFK tasks — no rogue operations while the player is away)
  - Brief disconnects (within 5 minutes) and reconnect resume watches automatically (old system deleted on disconnect)
  - Optimized via a global singleton event listener + reverse index — zero cost when nobody is subscribed; all of a player's condition watches merge into a single timer
  - **Watchable scope**: the AI only creates watches from the built-in monitorable list; anything outside it (e.g. external real-time data requiring network access, such as gold prices or exchange rates) is directly declined with an explanation — and even if the AI mispicks something outside the list, creation is rejected by the system's fallback validation (unsupported skill/action or event type returns a refusal reason), so a watch that could never fire simply cannot be created
  - Requires `kilacraft.watch` permission (default: all players). Per-player limits: 3 condition / 5 event watches / 200 server-wide
...
- **Chat Suggestions (highlight of this release)**: After the AI replies, 1-5 clickable "you may also want to ask" follow-up questions are appended below the chat, clickable to send as a command — no typing needed. Like "suggested questions" in chat apps, the AI hands you the next line after every answer, so the conversation keeps flowing without effort. Generated intelligently from multi-turn history + currently available skills, with a strict "quality over quantity" rule (if unsure, output nothing) — and as a bonus, **they double as an entry point for discovering what else the AI can do** (web search, watches, command execution, etc. can all appear as suggestions). **Triggers only in command mode, never in continuous-chat mode** — to preserve the natural conversational feel. Players can turn it off via `/kila suggestion off` (on by default, opt-out, no permission needed); server owners can configure count, timeout, excluded scenarios/skills, and display text in the `suggestion:` section of `behavior.yml`. Suggestion messages use the unified AI prefix; title and separator colors are customizable via config. Suggestions are laid out with intelligent width-adaptive wrapping (CJK and ASCII visual widths measured precisely), replacing the old fixed-column threshold

- **Command Execution Skill (CommandSkill, highlight of this release)**: Players can ask the AI in natural language to execute server commands on their behalf — "teleport to spawn", "claim my daily reward", "clear my chat history" — and the AI translates that into the actual server command and **runs it as that player** (gated by the server permission system, so the AI can never exceed the player's own permissions). This skill received a major upgrade in this version:
  - **Now on by default**: the skill is now enabled for all players by default (`kilacraft.command.execute` permission defaults to all players); previously it required manually setting `command_skill.enabled: true` in `config.yml` and the permission was OP-only. The toggle has been removed and control unified under the permission node. To disable, revoke `kilacraft.command.execute` via your permission plugin (e.g. LuckPerms)
  - **Command knowledge-base document**: a new `commands/commands.md` (Chinese + English) document serves as the AI's command-intent knowledge base. Ships with descriptions, examples, permissions, and keywords for all Kilacraft-AI's own commands; server owners can **append third-party plugin commands** (e.g. Essentials `home`/`tpa`) following the in-file template so the AI recognizes every command on the server. The parser is fault-tolerant — a single malformed entry is skipped with a warning rather than breaking the document. `/kila reload` refreshes the document; `/kila doctor` checks command-document health
  - **Permission-aware dynamic prompts**: the command list the AI sees is filtered by the current player's actual permissions — commands the player cannot use never enter the AI's context, so the AI only suggests/executes commands the player can actually run. Players with different permission ranks see different command lists

- **LLM Budget Governance**: A safety net across all LLM entry points. Tracks per-player hourly call counts; exceeding the threshold (default 200/hour/player, configurable via `budget_per_player_per_hour` in `llm.yml`) triggers a 1-hour circuit break. Uses **binary circuit breaking** — player-initiated requests (chat/skill/task) never trip and always respond even over budget; passive calls (greeting/suggestion/third-party plugin callbacks) are uniformly rejected during the trip window. Normal use never notices it (200/hour is unreachable in practice); it only kicks in under abnormal conditions (mob-farm signal storms, misconfiguration causing runaway calls, third-party plugin runaway) — **prioritizing player experience and the server owner's wallet**. Set to 0 or negative to disable entirely

- **LLM Cache Hit-Rate Statistics (`/kila cache`)**: A new `/kila cache` command to view real-time prompt-cache hit rates. Per-type breakdown across 10 AI call categories (intent recognition Phase 1 & 2, secondary analysis, normal chat, greeting, profile, suggestion, utility notifications, server diagnostics), each showing request count, input/output tokens consumed, and hit rate. Automatically detects cache metrics from DeepSeek (`prompt_cache_hit_tokens`), OpenAI (`cached_tokens`), and Anthropic (`cache_read_input_tokens`) API responses — no configuration needed. **In-game** output shows a single-screen bar chart per type with the global average hit rate in the header; **console** output is a self-aligning table (hit rate, request count, hit/input token detail, and an algorithm note). Statistics are session-scoped in-memory only (reset on restart); `/kila cache reset` subcommand for manual resets. Requires `kilacraft.admin.cache` permission (default: operators only)

### 🔧 Improvements

- **Major config consolidation**: the existing `greeting.yml` has been integrated with new suggestion, watch, and utility-notification configs into a single `behavior.yml` (four sections: greeting / suggestion / watch / utility.prompts), all adopting the single-file bilingual pattern. Individual ConfigManagers remain decentralized; `/kila reload` works seamlessly — fewer config files, simpler server-owner maintenance
- **Greeting system rebuilt**: login greetings no longer inject raw vanilla statistics (playtime, distances traveled, kill counts, etc.) or the last coordinates; instead only discrete highlight events from the last offline period (kills, boss kills, etc.) are kept. The tone shifts from a "status report" to a "casual recall" — the AI brings up your past exploits naturally, like an old friend, and can segue into related topics. Removing the high-frequency dynamic parameters (which change every tick) also stabilizes the greeting-prompt prefix and **significantly boosts cache hit rate on this path**
- **Bukkit query skill split**: the single generic Bukkit query skill (data-driven via `apis.yml`) has been split into 5 independent skills by responsibility (player info / player status / player inventory / world info / server info). All **71 read-only queries remain identical** (action IDs, return fields, output format unchanged). Each skill's prompt now includes reverse-boundary and disambiguation hints (e.g. "location" = feet coords vs "eye location" = eye coords), so the AI routes queries more accurately; the legacy `skills/bukkit/apis.yml` and `apis_en.yml` have been removed
- **Server health-monitor alert thresholds optimized**: MSPT anomaly detection moved from "single threshold breach" to a "max + 10-second median dual gate" (filters transient spikes, only alerting on sustained lag players can actually feel); CPU anomaly moved from "alert on first breach" to "confirm after 3 consecutive breaches" (debounce). Thresholds overall relaxed to reduce false positives: MSPT p95 threshold 50→100ms, CPU threshold 80%→90%; alert cooldown 5→30 minutes, max auto-analyses per window 5→2, window length 60→180 minutes. New config keys `mspt_median_threshold` (default 60) and `cpu_consecutive_threshold` (default 3) added
- **Automatic LLM model-name and URL compatibility**: a new compatibility resolver auto-maps legacy DeepSeek model names (`deepseek-chat`→`deepseek-v4-flash`, `deepseek-reasoner`→`deepseek-v4-pro`, **in-memory only — your config file is never touched**) so old configs keep working after upgrade without manual edits; it also auto-completes incomplete API URLs based on the OpenAI Chat Completions protocol (e.g. entering `https://api.deepseek.com/v1` auto-appends `/chat/completions`). Zero-config, transparent to non-DeepSeek setups
- **Unified dynamic-content injection across AI entry points**: per-player dynamic content scattered across AI call entry points (command lists, player metadata, profile context, etc.) has been extracted into a dedicated injection slot, keeping the cacheable prompt prefix stable across players and **further improving global cache hit rate**
- **system prompts made fully static (highlight of this release)**: going one step beyond the unified injection above — the system prompt for **all** LLM calls (normal chat, greeting, secondary analysis, etc.) no longer replaces any per-player placeholder (including `{player}`); it is now a byte-identical static text shared across all players, with player names, profile, offline events, current time and other dynamic data **all moved into the user message**. DeepSeek and similar providers match prefix cache "from byte 0 onward as a continuous same-prefix"; the `{player}` that used to sit in system broke the prefix right there — staticizing it noticeably raises cache hit rate in multi-player scenarios. A leftover `{player}` in an old config is no longer replaced (it stays in system as a literal, only lowering cache hit rate, not breaking functionality); if greeting config still contains deprecated placeholders, a warning is logged on startup prompting migration
...
- **Conversation history management hardened**: three improvements to `settings.max_history` in `config.yml` — **(1) value-range clamping**: values outside 0-100 are auto-clamped to the boundary (previously unlimited; an accidentally huge value could OOM); **(2) in-memory capacity now tracks the config**: the fixed in-memory capacity constant (hardcoded 100 entries) is removed and now tracks `max_history` (= max_history × 2 messages), so config is what you get; **(3) full disable semantics**: `max_history: 0` now fully disables history (no save, no load) and reconnect no longer loads any old records from the database. The DB reload amount is also adjusted to half the in-memory capacity (to avoid instantly filling the history queue). Changes via `/kila reload` take effect immediately (no restart)
...
- **Default context-round counts tuned**: intent-recognition history `intent_history_count` default 5→7 rounds, secondary-analysis history `analysis_history_count` default 2→3 rounds — longer history helps the AI resolve pronoun references in multi-turn dialogue (e.g. "check that item again") more accurately, and makes the suggestion system less likely to re-recommend topics the player already asked
...
- **Security hardening (WebFetch SSRF protection)**: the web-fetch feature has three layers of security built in, so server owners can enable it with confidence:
  - **Internal-address blocking**: by default, access to the server's own or LAN addresses (`127.x`/`10.x`/`192.168.x`/`172.16-31.x`) is forbidden — players cannot use the AI to probe the server's internal network
  - **DNS rebinding protection**: IP validation is welded directly into the DNS resolution step, guaranteeing "the IP that's checked is the IP that's connected" — eliminating the check-then-connect time window that's a common blind spot in similar plugins
  - **Forced HTTPS + per-hop redirect re-check**: when protection is on, `http://` is upgraded to `https://`; redirects are handled hop-by-hop manually (max 3 hops), re-validating protocol and IP on each hop to prevent "normal link → redirect to internal network" bypasses
...
- **Fixed a regression that broke the secondary-analysis history-count config**: the `agent.analysis_history_count` setting in `llm.yml` was accidentally broken by a previous version update — the secondary-analysis LLM call passed the full conversation-history queue, bypassing the configured round limit; it now truncates to the configured rounds again, avoiding token waste from overly long history
...
|------|----------------------|--------|
| `behavior.yml` | Brand new: replaces the former `greeting.yml`, integrating into four sections (greeting / suggestion / watch / utility.prompts) with single-file bilingual mode; the greeting prompt was rewritten to a "casual recall" tone. **After system-prompt staticization**, the dynamic placeholders `{player}`/`{offline_duration}`/`{own_events_section}`/`{friend_events_section}`/`{online_friends_section}`/`{last_session_highlights}` are all deprecated (dynamic data is now auto-injected into the user message; system keeps only `{server_info}` as the single cross-player static placeholder). UtilitySkill's LLM prompts also extracted here | ❌ Must delete `greeting.yml` then regenerate (old `greeting.yml` is now ignored; an old `behavior.yml` with deprecated placeholders leaves them as literals in system, lowering cache hit rate but not breaking functionality) |
| `intent_prompts.yml` / `_en.yml` | Intent recognition prompt ongoing optimization: added relative-adjustment rule (rule 10), parameter-missing handling (scenario 7), high-frequency-data historical-baseline notes; afk_task callback rules replaced by watch routing | ❌ Must delete (will route to the deleted afk_task system and miss new rule constraints otherwise) |
| `skills/` (all existing skill config files, including removed `skills/afktask/` and `skills/guardian/`) | All built-in Skill prompts restructured under a unified three-part format (summary + triggers + boundaries); UtilitySkill's LLM prompt sections moved to `behavior.yml`; the generic Bukkit query split into 5 independent skill configs (player info/status/inventory/world/server) and the legacy `skills/bukkit/apis.yml`, `apis_en.yml` removed; `skills/afktask/` and `skills/guardian/` removed along with their systems | ❌ Must delete the entire `skills/` folder and regenerate (old prompt format and the defunct apis.yml otherwise; intent recognition accuracy will drop significantly; old leftover files aren't loaded but take up space) |
| `language.yml` / `_en.yml` | Added `/kila cache` command text (redesigned hit-rate display text, type names); `/kila doctor` output now uses grouped fold-down summary format; added `kilacraft.admin.cache` permission node | 📋 Recommended (cache command text missing or shows placeholders, doctor shows old format otherwise) |
| `admin.yml` | Server health-monitor alert thresholds adjusted: added `mspt_median_threshold: 60`, `cpu_consecutive_threshold: 3`; `mspt_p95_threshold` 50→100, `cpu_threshold` 80→90, `cooldown_minutes` 5→30, `max_auto_analysis_per_window` 5→2, `auto_analysis_window_minutes` 60→180 | 📋 Recommended (old thresholds remain and alert more easily otherwise; new thresholds fall back to code defaults and don't break functionality) |
| `config.yml` | Removed the `afk_task` and `command_skill` config sections (command skill now controlled by permission node, on by default); `security.allowed_actions` drops `AFKTask.create_task` and adds `player_watch`; `social.skill_whitelist` default no longer contains `AFKTask`; `settings.max_history` now has a value range of 0-100 (out-of-range values are clamped) with clarified comments (0 = fully disable history) | ❌ Must delete (stale `command_skill` section lingers otherwise; if you had customized `allowed_actions`, manually add `player_watch`) |
| `llm.yml` | Added `budget_per_player_per_hour: 200` (LLM budget governance threshold); secondary-analysis system prompt now includes result-relevance filtering rule and wording-fidelity principle; `agent.intent_history_count` default 5→7, `agent.analysis_history_count` default 2→3; system prompt no longer contains the `{player}` placeholder (now purely static — player name injected via user message) | ✅ Optional (uses built-in defaults; a literal `{player}` in system lowers cache hit rate and slightly reduces response quality) |
| `output.yml` | The `afk_callback` scenario is merged into `skill_result` (watch/subscription notifications are the delivery of a player's deferred skill request; default main-chat channel) | ✅ Optional (no functional impact) |
...
5. **New permission nodes** (all default: all players, opt-in — players must enable them or server owner must configure to activate):
