> ⚠️ **归档文档（v2.2.0）**
>
> 本文档描述的**挂机任务系统已于 v2.2.0 整体移除**（`/kila afk` 命令、`AFKTaskManager`、`kilacraft.afk` 权限、`afk_task` 配置段、`skills/afktask/` 包全部删除）。
>
> 其能力已被三个新系统**功能替代**：
> - **守护系统（GuardianSkill）**：AI 主动看护的默认套餐（背包快满/装备耐久/背后威胁），开开关即生效
> - **玩家自定义监听（WatchSkill）**：用自然语言设定条件/事件监听，能力比旧系统更强
> - **跨玩家上下线订阅（PlayerWatchSkill）**：订阅好友上下线（替代旧系统的同名能力，支持多目标）
>
> 本文档仅作历史参考保留，正文不再维护。如需了解当前的事件监听能力，请参阅《内置 Skill 与 Event 能力清单》。
...
> **最后更新**: 2026-05-06  
> **说明**: 本文档详细介绍 AFK 挂机任务系统的架构、使用方式、调用链路和最佳实践（**已归档，系统已移除**）
