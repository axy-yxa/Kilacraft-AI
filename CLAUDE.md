...
Kilacraft-AI 是一个 **Minecraft LLM Agent 插件**：玩家在游戏内用自然语言对话，插件经两阶段意图识别路由到「技能（Skill）」执行，再用 LLM 做二次总结后输出。支持知识库 RAG 检索、玩家画像、挂机任务、第三方插件集成，兼容 Spigot/Paper/Folia。
...
4. StreamOutput → SoundEffect → **ResponsePipeline**（AIResponsePipeline）→ **LLMOutputCoordinator**（内含 LLMAnalysisService）
...
8. AFK 任务系统（条件）→ Admin 系统（ServerHealthGuardian 三重依赖：DB + 开关 + 诊断模型；Spark 检测失败 2 分钟重试）→ Metrics → 启动横幅（含异步 UpdateChecker）
...
| `command` / `command/impl` | `/kilacraft` 命令分发（`KilacraftCommand`）+ 各子命令处理器（reload/clear/chat/knowledge/plugins/personalities/afk/tasks/profile/notify/usage/history/memory/skills/run/doctor/about）+ TabCompleter |
| `common/enums` | 枚举（权限 `PluginPermissionEnum`、输出载体/场景、AFK 任务类型、对话来源等） |
...
| `config` | 各 ConfigManager（config/llm/output/knowledge/personalities/intent_prompts/greeting/admin/database/skill），`LanguageManager` |
...
| `listener` | 事件监听：`ChatListener`、`PrivateChatListener`、`TpaListener`、`AFKTaskListener`、`AdminListener` |
| `llm` | `LLMManager`、`LLMProvider` 接口、`GenericLLMProvider`（SSE 流式、在途取消、思考模式治理） |
| `metrics` | bStats 统计（`MetricsBootstrap`） |
| `model/**` | 数据模型：`afktask`/`bukkit`/`event`/`greeting`/`knowledge`/`notification`/`profile` |
...
所有命令示例与文档统一 `/kila`；`/kilacraft` `/ai` `/zm` 仅作别名列出（`/ai` 有冲突风险，不作示例）。`/kila plugins` **仅控制台可用**（玩家执行会被拒绝）。
...
- **`/kila reload`** 级联所有 ConfigManager；重载前快照 `isChinese`/`isEmbeddingEnabled`，变更则全量重建知识库（分块 + BM25 统计 + Embedding 缓存），并调 `TextProcessorFactory.reset()`。
